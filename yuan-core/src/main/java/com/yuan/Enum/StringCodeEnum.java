...
/**
 * All code-styled enum type should implement this interface to provide a consistent way to retrieve
 * the code value of this enum.
 * <p>
 * FIXME this pattern assumes the enums are static, i.e. when there is a new code value added in DB,
 * we'll need to update the enum class to include the new value. This requires a code change. To
 * overcome this limitation, we could possibly use a dynamic enum solution, e.g. article
 * http://niceideas .ch/roller2/badtrash/entry/java_create_enum_instances_dynamically, or
 * http://blog.xebia.com/2009/04/02/dynamic-enums-in-java/ Should later revisit this and evaluate
 * above options.
 * <p>
 * Created on 15-9-2.
 *
...
public interface StringCodeEnum extends CodeEnum {
   ...
   */
  static <E extends StringCodeEnum> E getEnumForCode(Class<E> cType,
      String code) {
    if (!cType.isEnum()) {
